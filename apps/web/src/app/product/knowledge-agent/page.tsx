          ...
          <div className="ka-hero__text">
            ...
            <div className="ka-hero__pain-point">
              ...
              <p>
                <strong>痛点：</strong>
                {
                  knowledgeAgentIntro.painPoint.split(
                    "约60%的申请人会因材料问题至少跑两趟",
                  )[0]
                }
                <span className="ka-text-highlight">
                  约60%的申请人会因材料问题至少跑两趟
                </span>
                {
                  knowledgeAgentIntro.painPoint.split(
            ...
            {productFeatures.map((feat, index) => (
              <div className="ka-feature-card" key={index}>
                <div className="ka-feature-icon-wrapper">
                  <span className="ka-feature-icon-text">
                    {feat.title.substring(0, 1)}
                  </span>
                </div>
                <h3>{feat.title}</h3>
                <p>{feat.desc}</p>
            ...
            <table className="ka-table">
              ...
              <tbody>
                {hardwareConfigs.rows.map((row, i) => (
                  <tr
                    key={i}
                    className={row.highlight ? "ka-tr-highlight" : ""}
                  >
                    ...
                    <td className="ka-td-desc">
                      {row.desc.split("\n").map((line, j) => (
                        <div
                          key={j}
                          className={
                            line.includes("HuaKun") ? "ka-model-name" : ""
                          }
                        >
                          {line}
                        </div>
