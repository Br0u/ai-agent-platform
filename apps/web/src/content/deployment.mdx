...
两条 DSN 分别使用 `ai_agent_control_migrator` 和 `ai_agent_control`，密码必须与对应密码文件一致。`MODEL_CONFIG_ENCRYPTION_KEY`、`AGENT_CONFIG_CONTROL_KEY`、`OS_SECURITY_KEY` 和认证密钥不得复用。助手聊天不创建服务端会话，只在当前页面 React 内存中保留上下文。
...
使用具有 `admin:assistant:configure` 权限的账号访问 `/admin/assistant`。选择 Provider 和部署允许的 Endpoint，填写 Model ID；Endpoint 要求 Key 时再填写 Key。先“保存草稿”，再“测试并启用”。测试成功后活动指针和运行时才会切换；保存本身不会影响当前模型。
...
- 查看明文 Key 需要独立权限。浏览器只显示 30 秒；隐藏 DOM 不能撤回已复制到操作系统剪贴板的内容。
...
<Callout type="warning">
  ...
  Key 提交到 Git，也不要写入日志或工单。
</Callout>

## 当前能力边界

后台已经支持六家云模型配置，以及“已验证 ZIP → 码多多运行时”的 Skill 闭环。当前不支持 GitHub/GitLab/GitCode 导入，也没有比容器边界更细的脚本沙箱；因此只允许上传通过自动安全扫描的 Skill。Knowledge、网页操作和本地算力仍是 roadmap 入口。
