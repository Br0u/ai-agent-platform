...
export const capabilityFoundationSlugs = [
  "agent-knowledge-base",
  "knowledge-metrics",
] as const;

const capabilityFoundations = [
  {
    ...
    cta: {
      ...
      description: "面向企业知识库建设与数据指标体系建设需求，与华鲲团队沟通。",
      actions: [
        {
          label: "商务咨询",
    ...
    sections: [
      {
        ...
        visual: "「数据源 → 数据同步 → 指标开发 → 智能问数」链路示意图素材槽位",
      },
      {
        eyebrow: "02｜能力优势",
        title: "数据底座能帮你做什么",
      ...
      },
      {
        ...
        cards: [
          {
            title: "数据智能体",
            description: "智能问数直接建立在数据源与指标之上，问得准、答得快。",
            actions: [
              {
                label: "查看数据智能体 →",
                href: "/product/data-agent",
...
export function getCapabilityFoundation(
  slug: string,
): PlatformPage | undefined {
  return capabilityFoundations.find((page) => page.slug === slug);
}
