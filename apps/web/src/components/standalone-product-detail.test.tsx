...
describe("StandaloneProductDetail", () => {
  it.each([
    ...
    ({ cta, scene, securityCount, slug, title }) => {
  ...
  it("does not repeat the code-agent hero demo as an experience section", () => {
    render(<StandaloneProductDetail slug="code-agent" />);

    expect(
      screen.queryByRole("heading", {
        level: 2,
        name: "说需求 → 分析项目上下文 → 生成代码 → 运行验证",
      }),
    ).not.toBeInTheDocument();
    expect(screen.queryByTestId("detail-experience-flow")).toBeNull();
  });
  ...
  it.each([
    ...
    "keeps the $slug prototype experience section",
    ({ flow, slug, title }) => {
      render(<StandaloneProductDetail slug={slug} />);
