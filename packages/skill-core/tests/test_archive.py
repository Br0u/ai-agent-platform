...
def test_ignores_macos_metadata_after_validating_and_counting_it(zip_builder) -> None:
    metadata = {
    ...
    assert canonical.extracted_size == len(DEFAULT_SKILL_MD) + sum(
        len(content) for content in metadata.values()
    )
