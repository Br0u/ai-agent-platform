...
恢复脚本内只有一个 Docker CLI 监督器。decrypt、bundle validation、dump digest、PostgreSQL 和 Skill Registry migration 都使用唯一名称，统一执行 register → bounded create → start → wait/exec → EXIT reconcile；数据库卷也在 create 请求前注册。禁止匿名`docker run --rm`与`docker run -d`。每次 Docker 调用都显式分离到`0600`stdout/diagnostic 文件；只有 exact resource name、wait status、digest、计数和 schema contract 等预期非敏感标量会被解析，诊断、archive、源码、Secret 和宿主路径不会回显。单次 create 上限为`RESTORE_DOCKER_CREATE_TIMEOUT_SECONDS=30`，其他 CLI 上限为`RESTORE_DOCKER_CLI_TIMEOUT_SECONDS=10`；超时先 TERM，`RESTORE_DOCKER_CLI_KILL_AFTER_SECONDS=2`后 KILL 并 wait。容器内解密/校验/digest/迁移等待受`RESTORE_DECRYPT_TIMEOUT_SECONDS=3600`约束。监督器只依赖已探测可用的`sh`、`kill`、`wait`和`sleep 0.1`，不依赖 GNU timeout 或 setsid。

资源注册表位于受保护临时目录内，只保存非敏感 key/type/exact name/create outcome，目录为`0700`、文件为`0600`。create 请求前 outcome 为 AMBIGUOUS；只有命令退出 0 才改为 SUCCESS。create 请求一旦启动，任何非零、超时、signal、连接/响应失败或被监督器终止都继续保持 AMBIGUOUS；通用 CLI 退出状态不能证明 daemon 未创建资源。SUCCESS 清理只有 bounded remove 成功，或 bounded exact-name 查询连续两次成功空输出，才能确认完成；精确名称为 EXISTS，超时、非零或意外输出为 UNKNOWN，均不会累计 absent。`RESTORE_DECRYPT_RECONCILE_ATTEMPTS=3`合法范围是 2..10。AMBIGUOUS 不允许被快速 ABSENT 擦除：脚本在`RESTORE_DOCKER_CREATE_SETTLE_SECONDS=5`（合法范围 1..300）的 wall-clock 窗口内持续 exact query/remove，只有 bounded remove 成功才能收敛；窗口结束仍未明确移除就固定输出`restore drill cleanup failed`。已开始的单次 CLI 最多可在窗口后再占用自身 timeout 加 KILL 宽限。
