...
恢复测试只允许使用隔离数据库、临时目录和测试 Secret。执行前准备`0600`普通文件`BACKUP_DATABASE_PASSWORD_FILE`与`BACKUP_ENCRYPTION_KEY_FILE`，设置测试镜像`BACKUP_CRYPTO_IMAGE`、`RESTORE_SKILL_REGISTRY_IMAGE`和隔离临时根`RESTORE_TMP_ROOT`。解密前用`RESTORE_MAX_ENCRYPTED_BYTES=34359738368`限制密文；恢复根可用空间必须覆盖密文、两倍解密上限以及`RESTORE_SPACE_SAFETY_BYTES=67108864`安全余量，不足时在创建 Docker 资源前固定失败。
