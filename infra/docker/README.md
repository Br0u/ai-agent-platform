...
`ai_agent_control_migrator`拥有`agent_control`schema 和表，`ai_agent_control`只获得运行所需的`SELECT/INSERT/UPDATE`权限。部署环境中的完整 Provider/Model/Key 仍是只读 bootstrap；一旦存在动态活动配置，动态配置优先，加载失败也不会静默回退。自定义模型 Endpoint 只能编辑`infra/agent/model-endpoints.json`后重建 Agent 镜像；目录以 root `0644`复制到只读容器，Web 不持有目录或模型 Endpoint。
