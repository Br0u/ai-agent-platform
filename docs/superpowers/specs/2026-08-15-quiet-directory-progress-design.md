...
- Desktop directories start at 52px wide.
...
- Expanding is always explicit. Hover and scrolling never open the directory.
...
- The dot may use a short positional transition, disabled under `prefers-reduced-motion: reduce`.
...
- Browser tests at 1440px, 901px, 900px, 800px, and 390px: equal 52px desktop rails above the boundary; complete modal behavior at and below 900px; no horizontal overflow; manual expansion; active tracking including the final short section; active-ancestor expansion; existing mobile drawers; keyboard focus; and one unchanged Agent launcher.
