...
it('presents the product and primary documentation action', () => {
  render(<HomePage />)
  expect(screen.getByRole('heading', { name: /AI Agent Platform/i })).toBeInTheDocument()
...
it('returns FEATURE_DISABLED for disabled modules', () => {
  expect(getFeatureStatus('license', false)).toEqual({
    module: 'license', enabled: false, mode: 'placeholder', errorCode: 'FEATURE_DISABLED'
  })
})
```
...
- [ ] **Step 5: Review sensitive files and Git diff**
