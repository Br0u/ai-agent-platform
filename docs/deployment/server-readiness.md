...
```bash
cp .env.example .env
# 分别生成平台 owner/migrator/runtime/backup、Agno migrator/runtime、Better Auth 和 AgentOS 密钥；不要复用
...
数据库角色分离：平台 owner 只供初始化/受控升级；平台 migrator 只供迁移；平台 runtime 只供 Web；Agno migrator/runtime 只能访问`agno`；backup 只读`public`、`drizzle`、`agno`。`BACKUP_DATABASE_URL`不得提供给 Web 或 Agent。运行时账号均无 schema 变更权限。`web`和`agent`都没有主机端口，唯一公开入口是 Nginx `proxy`。
...
发布前可执行一次完整的当前镜像演练；脚本使用隔离 Compose 项目、随机`0600`密钥文件，验证 AgentOS readiness 与真实 dump/restore，并在任何退出路径删除容器、卷和临时文件：
