...
Web 仅接收运行时数据库 URL、Better Auth 密钥、AgentOS Bearer 密钥、助理会话签名密钥和助理限流密钥。它不得接收数据库 owner、平台 migrator 或 Agno migrator 凭据。`ASSISTANT_PUBLIC_ORIGIN`必须是对外 HTTPS 精确 origin；仅隔离验收脚本允许`http://127.0.0.1:8080`并签发开发 Cookie。
