...
# BACKUP_ENCRYPTION_KEY_FILE 指向仓库外或已忽略的 0600 文件，内容至少 32 个随机字符
...
备份容器以 PostgreSQL 的非 root 用户运行，先在 tmpfs 生成 custom-format dump，再使用 AES-256-CBC、PBKDF2、600000 次迭代加密，最终只把`0600`的`.dump.enc`文件原子写入独立卷。数据库密码只经临时`PGPASSFILE`传递；明文 dump 和凭据文件在成功、失败及信号退出时都会清理。每次发布前至少复制一份加密备份到受控主机，使用同一份外部加密密钥执行隔离恢复演练：
...
BACKUP_CRYPTO_IMAGE=ai-agent-platform-backup:latest \
  ...
  /secure/path/ai-agent-platform-YYYYMMDDTHHMMSSZ.dump.enc \
  EXPECTED_USERS EXPECTED_AGNO_SESSIONS USER_FIXTURE_ID AGNO_SESSION_FIXTURE_ID
```

脚本先在受保护临时目录解密，再创建临时数据库卷和容器，恢复后验证平台迁移历史、关键表、`agno_sessions`、`agno_schema_versions`及非敏感行数，最后删除明文、临时凭据、容器和卷；它不会把密钥、数据库密码或消息正文放进命令参数和日志。生产备份仍必须异机复制并设置失败告警。加密密钥必须由独立密钥管理系统保管；丢失该密钥将导致历史备份不可恢复。

发布前可执行一次完整的当前镜像演练；脚本使用隔离 Compose 项目、随机`0600`密钥文件，验证 AgentOS readiness、真实加密 dump/restore 及错误密钥必然失败，并在任何退出路径删除容器、卷、本项目本地构建镜像和临时文件：
