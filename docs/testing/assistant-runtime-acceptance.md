...
- 第一阶段结束后立即独立扫描其容器日志。所有环境凭据、E2E 密码和 session token、AgentOS 运行令牌、模型夹具、派生数据库 URL 及 Secret 文件路径均写入临时 `0600` pattern 文件，使用 `grep -F -f` 检查，凭据不进入命令参数或输出。
- 第二阶段在同一数据库卷内强制重建 `agent`、`web` 和 `proxy`，切换为 `ASSISTANT_PROVIDER_MODE=agentos`、`AGENT_ENABLED=true`，重新播种固定鉴权夹具并重置 Nginx 限流状态；完成 4 条 AgentOS 测试后再次独立扫描日志。
- AgentOS 阶段只注入验收镜像内的离线确定性模型；验收 Agent 只连接内部 `backend` 网络，不连接 `model_egress`，关闭 Agno telemetry，且测试对任意 socket 网络访问 fail fast。生产镜像仍以最后一个 `runtime` target 为默认产物，且不包含验收包。
