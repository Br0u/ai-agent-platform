...
最终结果：UI 75 passed；Web 749 passed / 35 skipped；Database 104 passed / 12 skipped；typecheck、lint、format、build 均 exit 0。Database skipped 项为未提供 `TEST_DATABASE_URL` 的破坏性集成测试，Docker E2E 使用独立数据库补足浏览器链路验收。
...
reduced-motion 下莫比乌斯动画为静态，抽屉没有活动的 transform transition。纯键盘检查覆盖顶栏入口、浮动入口、抽屉输入、完整工作区链接、完整页 composer、后台导航和认证表单；所有目标均显示非零 focus outline。
